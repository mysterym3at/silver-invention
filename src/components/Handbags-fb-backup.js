...
useEffect(() => {
  const fetchData = async () => {
    // 1. Fetch collection and its joined designs
    const collectionRes = await fetch(`${API_BASE}/api/collections/${COLLECTION_ID}`);
    const collectionData = await collectionRes.json();
    setHandbags([collectionData]); // Assuming you store collections in this state

    // 2. Fetch all shapes for the dropdown/lookup
    const shapesRes = await fetch(`${API_BASE}/api/shapes`);
